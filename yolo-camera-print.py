...
# Creating a logging xlsx
workbook = xlsxwriter.Workbook('Log.xlsx')
worksheet = workbook.add_worksheet("Log sheet")
worksheet.write('A1','Frame collected at')
worksheet.write('B1', 'Objects')
...
while True:
    ...
    cameraframe = datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
    worksheet.write(row, col, cameraframe);
    print("frame collected at  ",cameraframe);
    ...
    if len(results) > 0:
        ...
        for i in results.flatten():
            ...
            col+=1
            worksheet.write(row, col, text_box_current);
